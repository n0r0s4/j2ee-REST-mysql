...
public class CoursesResource {
    ...
    public CoursesResource(@Context ServletContext context) {
        ...
        if (context.getAttribute("studentService") != null)
                studentService = (TeacherService) context.getAttribute("studentService");
        else {
                studentService = new TeacherService();
    ...
    public Response getCourseStudents(@PathParam("id") int id) {
        List<Teacher> courseStudents = (List<Teacher>) courseService.getCourseStudents(id);

        if (courseStudents == null)
                return Response.status(Response.Status.NOT_FOUND).build();
        else {

            GenericEntity<List<Teacher>> entity = new GenericEntity<List<Teacher>>(courseStudents) {
            };
    ...
    public Response addStudentToCourse(@PathParam("id_student") int idStudent, @PathParam("id_course") int idCourse) {
        ...
        Teacher student = studentService.getStudentById(idStudent);
        if (student == null)
                return Response.status(Response.Status.NOT_FOUND).build();
